...
export class DevRequirementsWorkflow {
	...
	async runSoftwareDeveloperWorkflow(requirements: string): Promise<void> {
		...
		let repoPath = await agentContextStorage.getStore().scm.cloneProject(gitLabProject.path_with_namespace);
		...
		await agentContextStorage.getStore().scm?.createMergeRequest(mrTitle, mrDescription);
	...
	async selectProject(requirements: string): Promise<GitLabProject> {
		const projects = await agentContextStorage.getStore().scm.getProjects();
		const prompt = buildPrompt({
