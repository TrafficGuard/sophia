...
2. The secondary supporting files which contain documentation and type information (interfaces, types, classes, function, consts etc) that will be required to correctly makes the changes..
...
`;
	const response = (await llms().hard.generateTextAsJson(prompt)) as SelectFilesResponse;
